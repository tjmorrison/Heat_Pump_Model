...
nw = Network(
    fluids=['air', 'R744', 'water'], T_unit='C', p_unit='bar', h_unit='kJ / kg',
    m_unit='kg / s'
)
# %% components

# First argument of the component class is the label (name)
# so this chunk of code is just naming each component of our system 
...
# consumer system
c_in_cd = Connection(cc, 'out1', cd, 'in1') # coolent into condenser
cb_dhp = Connection(cc_cons, 'out1', dhp, 'in1') #from condenser to pump
dhp_cd = Connection(dhp, 'out1', cd, 'in2') #from pump to condenser inlet 2?
cd_cons = Connection(cd, 'out2', cons, 'in1') #from condenser to heatexchanger to consumer
cons_cf = Connection(cons, 'out1', cc_cons, 'in1') #heat exchanger to consumer cycle

nw.add_conns(c_in_cd, cb_dhp, dhp_cd, cd_cons, cons_cf) #makes the connections between each conneector class

# connection condenser - evaporator system
cd_ves = Connection(cd, 'out1', ves, 'in1') #from condenser to evaporator valve

nw.add_conns(cd_ves) #adds the connection

# evaporator system
ves_dr = Connection(ves, 'out1', dr, 'in1') #from valve to drum(?)
dr_erp = Connection(dr, 'out1', erp, 'in1') #from drum to pump
erp_ev = Connection(erp, 'out1', ev, 'in2') #from pump to heat exchanger on evaporator
ev_dr = Connection(ev, 'out2', dr, 'in2') #from heatexchanger to drum
dr_su = Connection(dr, 'out2', su, 'in2') #from drum to superheater

nw.add_conns(ves_dr, dr_erp, erp_ev, ev_dr, dr_su) #complete connections
...
cd.set_attr(pr1=0.99, pr2=0.99, ttd_u=5, design=['pr2', 'ttd_u'],
            offdesign=['zeta2', 'kA_char']) # set the attributes 
...
# condenser system
c_in_cd.set_attr(fluid={'air': 0, 'R744': 1, 'air': 0})
cb_dhp.set_attr(T=50, p=10, fluid={'air': 0, 'R744': 0, 'air': 1})
...
amb_fan.set_attr(T=12, p=1, fluid={'air': 1, 'R744': 0, 'air': 0},
                 offdesign=['v'])
